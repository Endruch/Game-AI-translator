...
class TranslationWorker(QObject):
    ...
    def __init__(self, rect, source_lang, target_lang, api_key, color_filters, check_changes=False):
        super().__init__()
        self.rect = rect
    ...
    def run(self):
        if self.check_changes:
            changed, text = capture_if_changed(self.rect.x(), self.rect.y(), self.rect.width(), self.rect.height(), self.color_filters)
            if not changed:
                return
        else:
            text = capture_and_recognize_sync(self.rect.x(), self.rect.y(), self.rect.width(), self.rect.height(), self.color_filters)
            if not text:
...
class App:
    ...
    def _setup_tray(self):
        ...
        menu = QMenu()
        menu.setStyleSheet("QMenu { background: #1e1e30; color: white; border: 1px solid #3a3a6a; } QMenu::item:selected { background: #3a3a6a; }")
        ...
        self.tray.activated.connect(lambda r: self._show_all() if r == QSystemTrayIcon.ActivationReason.DoubleClick else None)
        self.tray.show()
    ...
    def _start_translation(self, check_changes):
